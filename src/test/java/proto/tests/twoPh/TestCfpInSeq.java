 ...
 */
public class TestCfpInSeq extends Test {
  ...
  public Behaviour load(Agent a, DataStore ds, String resultKey) throws TestException {
    ...
    r1 = TestUtility.createAgent(a, R1, "test.proto.tests.twoPh.TestCfpInSeq$Responder", null);
    r2 = TestUtility.createAgent(a, R2, "test.proto.tests.twoPh.TestCfpInSeq$Responder", null);
    ...
    // Create and return the behaviour that will actually perform the test
  	Behaviour b = new TwoPhInitiator(a, cfp) {
  		int cnt = 0;
        ACLMessage r1Propose = null;
        ACLMessage r2Cfp = null;
        int step = 0;
        Vector ph1Msgs = new Vector();

  		protected void handlePropose(ACLMessage propose) {
              l.log("\n\nLOG - (TwoPhInitiator, handlePropose(), " + myAgent.getLocalName() + ") - " + propose);
              switch(step) {
                  case 0:
                      if(propose.getSender().getLocalName().equals(r1.getLocalName())) {
                          r1Propose = propose;
                          step++;
                      }
                      else {
                          l.log("\n\nLOG - (TwoPhInitiator, handlePropose(), " + myAgent.getLocalName() + ") - " +
                                "propose received from " + propose.getSender() + " while r1 was expected.");
                          success = success && false;
                      }
                      break;
                  case 1:
                      if(!propose.getSender().getLocalName().equals(r2.getLocalName())) {
                          l.log("\n\nLOG - (TwoPhInitiator, handlePropose(), " + myAgent.getLocalName() + ") - " +
                                "propose received from " + propose.getSender() + " while r2 was expected.");
                          success = success && false;
                      }
                      step++;
                      break;
	    ...
	    protected void handleAllPh0Responses(Vector responses, Vector proposes, Vector pendings, Vector nextPhMsgs) {
	    	...
	    	if (proposes.size() != 1) {
                l.log("\n\nLOG - (TwoPhInitiator, handleAllPh0Responses(), " + myAgent.getLocalName() + ") - " +
                        proposes.size() + " proposes received while 1 was expected.");
	    		error = true;
	    	}
	    	if (pendings.size() != 0) {
                l.log("\n\nLOG - (TwoPhInitiator, handleAllPh0Responses(), " + myAgent.getLocalName() + ") - " +
	    	...
	    	if (cnt != 1) {
                l.log("\n\nLOG - (TwoPhInitiator, handleAllPh0Responses(), " + myAgent.getLocalName() + ") - " +
                        cnt + " query-if messages prepared while 1 was expected.");
	    		error = true;
	    	}
            switch(step) {
                case 1:
                    ph1Msgs.addAll(nextPhMsgs);
                    nextPhMsgs.clear();
                    r2Cfp = new ACLMessage(ACLMessage.CFP);
                    r2Cfp.setProtocol(TwoPhConstants.JADE_TWO_PHASE_COMMIT);
                    r2Cfp.addReceiver(r2);
                    r2Cfp.setReplyByDate(new Date(System.currentTimeMillis() + 600000)); // 30 sec timeout
                    r2Cfp.setConversationId(r1Propose.getConversationId());
                    nextPhMsgs.add(r2Cfp);
                    TwoPh0Initiator ph0 = (TwoPh0Initiator) this.getPhase(TwoPhInitiator.PH0_STATE);
                    ph0.getDataStore().put(ph0.ALL_CFPS_KEY, nextPhMsgs);
                    break;
                case 2:
                    nextPhMsgs.addAll(ph1Msgs);
                    break;
            }
	    	if (error) {
	    		nextPhMsgs.clear();
